...
- **슬래시 커맨드 10종** — `/gdc-login` `/gdc-switch` `/gdc-my-tasks` `/gdc-tasks` `/gdc-task-new` `/gdc-task-from-doc` `/gdc-doc-from-task` `/gdc-link-task` `/gdc-sync` `/gdc-update`. (**Claude Code 전용**)

...
| `search_tasks` | 현재 프로젝트 키워드·다중 필터 검색(상태/우선순위/유형/담당자/관련자/고객사/종료일 범위). 완료 태스크도 기본 포함, 결과는 번호 내림차순(관련도순 아님) |
...
| `get_task` | 태스크 상세 조회(하위·연관·상위 태스크 + 실제 날짜·고객사·비중·관련자·태그·댓글 수 등 수정 가능 필드 포함) |
...
| `restore_task` | 휴지통 태스크 복구(관리자 이상, `confirm` 게이트) |
